<?php
$isEdit = !empty($page);
$action = $isEdit ? site_url('admin/pages/update/'.$page['id']) : site_url('admin/pages/store');
$base   = rtrim(base_url(), '/');

// valeur courante (image principale)
$currentImageRel = old('image_url', $page['image'] ?? '');
$currentImageAbs = $currentImageRel
    ? (str_starts_with($currentImageRel, 'http') ? $currentImageRel : $base.'/'.$currentImageRel)
    : '';
?>
<div class="container py-4">
    <h3 class="mb-3"><?= $isEdit ? 'Modifier la page' : 'Créer une page' ?></h3>
    ...
    <form action="<?= $action ?>" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control" required
                   value="<?= esc(old('title', $page['title'] ?? '')) ?>">
        </div>

        <div class="mb-3">
            <label class="form-label">Slug (URL)</label>
            <input type="text" name="slug" class="form-control"
                   value="<?= esc(old('slug', $page['slug'] ?? '')) ?>">
        </div>

        <div class="mb-3">
            <label class="form-label">Description</label>
            <textarea name="description" rows="3" class="form-control"><?= esc(old('description', $page['description'] ?? '')) ?></textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                <input type="text" name="image_url" id="pageMainImageUrl" class="form-control"
        ...
        <div class="mb-3">
            ...
            <textarea id="editorContent" name="content"><?= esc(old('content', $page['content'] ?? '')) ?></textarea>
        ...
        <div class="d-flex gap-2">
            <a href="<?= site_url('admin/pages') ?>" class="btn btn-outline-secondary">Retour</a>
...
<script>
    ...
    function applySelected(rel, absUrl){
        ...
        if (mediaPickerMode === 'main') {
            ...
            document.getElementById('pageMainImageUrl').value = rel;
            const img = document.getElementById('pageMainImagePreview');
            img.src = absUrl;
