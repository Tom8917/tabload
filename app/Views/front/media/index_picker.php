...
function isImageRowPicker(array $f): bool
{
    ...
    // new: mime_type/type (DB-only) ou mime_type/kind (si tu l'as encore)
    $mime = strtolower((string)($f['mime_type'] ?? ''));
    $type = strtolower((string)($f['type'] ?? '')); // image|document (ta nouvelle archi)
    $kind = strtolower((string)($f['kind'] ?? '')); // compat

    return $type === 'image' || $kind === 'image' || str_starts_with($mime, 'image/');
}

function fileUrlPicker(array $f): string
{
    // legacy : url absolue déjà fournie
    if (isset($f['url'])) return (string)$f['url'];

    // new DB-only : toujours passer par endpoint controller
    $id = (int)($f['id'] ?? 0);
    return $id > 0 ? site_url('media/file/' . $id) : '#';
}
...
function currentNavUrlPicker(?int $folderId, string $filter, string $sort): string
{
    ...
    $pick = (string)($_GET['pick'] ?? 'image');
    return $u . '?picker=1&pick=' . rawurlencode($pick) . '&type=' . rawurlencode($filter) . '&sort=' . rawurlencode($sort);
}
?>
<!doctype html>
<html lang="fr">
<head>
    ...
    <style>
        /* petits bonus iframe : fond + lisibilité */
        ...
        .thumb { height: 140px; width: 100%; object-fit: cover; display:block; background:#f0f1f5; }
...
<div class="container-fluid py-3">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
            <?php if (!$isLegacy): ?>
                <a class="chip <?= $filter === 'all' ? 'active' : '' ?>"
                   href="<?= esc(currentNavUrlPicker($currentFolderId, 'all', $sort)) ?>">Tous</a>

                <a class="chip <?= $filter === 'image' ? 'active' : '' ?>"
                   href="<?= esc(currentNavUrlPicker($currentFolderId, 'image', $sort)) ?>">
                    <i class="fa-solid fa-image"></i> Images
                </a>

                <a class="chip <?= $filter === 'document' ? 'active' : '' ?>"
                   href="<?= esc(currentNavUrlPicker($currentFolderId, 'document', $sort)) ?>">
        ...
        <div class="card-soft mb-3">
            <div class="p-3">
                ...
                <div class="mt-3 d-flex flex-wrap gap-2 align-items-center">
                    ...
                    <div class="ms-auto d-flex gap-2">
                        <button id="btnClear" type="button" class="btn btn-outline-secondary rounded-pill" disabled>
                            Vider
                        </button>
                        <button id="btnUpload" type="button" class="btn btn-success rounded-pill" disabled>
                            Uploader
                        </button>
                    </div>
                </div>
    ...
    <!-- Fichiers -->
    <div class="d-flex align-items-center mb-2">
        <div class="h6 mt-3">Fichiers</div>
        <div class="ms-auto text-muted small">
            <span id="filesCount"><?= count($newFiles ?? []) ?></span> fichier(s)
        </div>
    </div>

    <?php $filesToShow = $isLegacy ? $legacyFiles : $newFiles; ?>

    <div class="row g-3" id="filesGrid">
        <?php if (empty($filesToShow)): ?>
            <div class="col-12">
                <div class="text-muted">Aucun fichier ici.</div>
            </div>
        <?php else: ?>
                    ...
                    <div class="card-soft file-card h-100">
                        ...
                        <?php else: ?>
                            <div class="d-flex align-items-center justify-content-center" style="height:140px;">
                                <div class="text-muted small"><i class="fa-solid fa-file-lines me-2"></i>Document</div>
                            </div>
                        <?php endif; ?>
                                ...
                                <?php if (!$isFilePicker): ?>
                                    <?php if ($isImg): ?>
                                        <button type="button"
                                                ...
                                                onclick="selectImage('<?= esc($url, 'attr') ?>','<?= esc($name, 'attr') ?>')">
                                            Utiliser
                                        </button>
                                    <?php else: ?>
                                        <button type="button"
                                                class="btn btn-primary btn-sm rounded-pill flex-grow-1"
                                                onclick="selectFile(<?= (int)($f['id'] ?? 0) ?>,'<?= esc($name, 'attr') ?>','<?= esc($url, 'attr') ?>')">
                                            Utiliser
                                        </button>
                                    <?php endif; ?>
                                <?php else: ?>
...
<script>
     ...
     * - Envoie toujours: { type:'MEDIA_PICKED', media:{ id,name,url,kind } }
     */
    (function () {
        ...
        window.selectImage = function (absUrl, name) {
            postToParent({
                type: 'MEDIA_PICKED',
                media: { id: null, name: name || '', url: absUrl || '', kind: 'image' }
            });
            tryClose();
        };

        window.selectFile = function (id, name, url) {
            postToParent({
                type: 'MEDIA_PICKED',
                media: { id: Number(id || 0), name: name || '', url: url || '', kind: 'document' }
            });
            tryClose();
        };
        ...
        (function () {
            ...
            let queue = [];
            const maxSize = 5 * 1024 * 1024; // ✅ aligné avec "5 Mo"
